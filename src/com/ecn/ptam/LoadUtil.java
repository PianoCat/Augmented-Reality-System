...
public class LoadUtil 
{
    ...
    public static LoadedObjectVertexNormalTexture loadFromFile
    (String fname, Resources r,MySurfaceView mv,float x,float y,float z)
    {
    	//加载后物体的引用
    	LoadedObjectVertexNormalTexture lo=null;
    	//原始顶点坐标列表--直接从obj文件中加载
		    ...
		    while((temps=br.readLine())!=null) 
		    {
		    	...
		    	if(tempsa[0].trim().equals("vn"))
		    	{
		    		aln.add(Float.parseFloat(tempsa[1]));
		      		...
		      		tempsa=temps.split("[ ]+");
		      	    //若为顶点坐标行则提取出此顶点的XYZ坐标添加到原始顶点坐标列表中
		      		alv.add(Float.parseFloat(tempsa[1])-x);
		      		alv.add(Float.parseFloat(tempsa[2])-y);
		      		alv.add(Float.parseFloat(tempsa[3])-z);
		      		
		      		alt.add(Float.parseFloat(tempsa[4]));
		      		alt.add(Float.parseFloat(tempsa[5])); 
		      		alt.add(Float.parseFloat(tempsa[6])); 		      		
		      	}

		      	else if(tempsa[0].trim().equals("f")) 
		      	{//此行为三角形面
		      		
		      		int indexTex;//三个顶点索引值的数组
		      		
		      		//计算第0个顶点的索引，并获取此顶点的XYZ三个坐标	      		
		      		indexTex=Integer.parseInt(tempsa[1].split("/")[0])-1;
		      		int x0=3*indexTex;
		      		int y0=3*indexTex+1;
		      		int z0=3*indexTex+2;
		      		alvResult.add(alv.get(x0));
		      		alvResult.add(alv.get(y0));
		      		alvResult.add(alv.get(z0));
		      		
		      		altResult.add(alt.get(x0));
		      		altResult.add(alt.get(y0));
		      		altResult.add(alt.get(z0));
		      		
		      		alnResult.add(aln.get(x0));
		      		alnResult.add(aln.get(y0));
		      		alnResult.add(aln.get(z0));
		      		
		      	    //计算第1个顶点的索引，并获取此顶点的XYZ三个坐标	  
		      		indexTex=Integer.parseInt(tempsa[2].split("/")[0])-1;
		      		int x1=3*indexTex;
		      		int y1=3*indexTex+1;
		      		int z1=3*indexTex+2;
		      		alvResult.add(alv.get(x1));
		      		alvResult.add(alv.get(y1));
		      		alvResult.add(alv.get(z1));
		      		
		      		altResult.add(alt.get(x1));
		      		altResult.add(alt.get(y1));
		      		altResult.add(alt.get(z1));
		      		
		      		alnResult.add(aln.get(x1));
		      		alnResult.add(aln.get(y1));
		      		alnResult.add(aln.get(z1));
		      		
		      	    //计算第2个顶点的索引，并获取此顶点的XYZ三个坐标	
		      		indexTex=Integer.parseInt(tempsa[3].split("/")[0])-1;
		      		int x2=3*indexTex;
		      		int y2=3*indexTex+1;
		      		int z2=3*indexTex+2;
		      		alvResult.add(alv.get(x2));
		      		alvResult.add(alv.get(y2));
		      		alvResult.add(alv.get(z2));
		      		
		      		altResult.add(alt.get(x2));
		      		altResult.add(alt.get(y2));
		      		altResult.add(alt.get(z2));
		      		
		      		alnResult.add(aln.get(x2));
		      		alnResult.add(aln.get(y2));
		      		alnResult.add(aln.get(z2));	      				      			      
