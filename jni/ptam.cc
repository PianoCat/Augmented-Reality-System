...
//按下按钮计数
JNIEXPORT jboolean JNICALL Java_com_ecn_ptam_PTAM_drawGrid(JNIEnv * env,jobject thiz)
{

	System *s=System::get_instance();
	return (jboolean)s->drawGrid();
}
...
JNIEXPORT jfloatArray JNICALL Java_com_ecn_ptam_PTAM_getFrustumMatrix(JNIEnv * env,jobject thiz){
	System *s=System::get_instance();
	float *mat=s->get_frustummatrix();
	jfloatArray ret=env->NewFloatArray(16);
	env->SetFloatArrayRegion(ret,0,16,mat);
	delete mat;
	return ret;
